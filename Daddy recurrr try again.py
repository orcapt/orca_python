# -*- coding: utf-8 -*-
"""
Created on Wed Aug 17 19:03:32 2016
...
order5Seq={}
for id in sequences:  
    if id in order2Seq or id in order3Seq or id in order4Seq:
        continue
    seq = sequences[id]
    coeff = checkRecurrence(seq,5)
    if coeff!=None:
        predict = predictNextTerm(seq, coeff)
        order5Seq[id]=(predict,coeff)

print ("We found %d sequences \n" %len(order5Seq))

for key in sorted(order5Seq):
    value = order5Seq[key]
    real.append([key, value[0]])
