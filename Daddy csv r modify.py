# -*- coding: utf-8 -*-
"""
Created on Mon Aug 22 19:00:10 2016
...
for key in sorted(order2Seq):
    value = order2Seq[key]
    if [key, value[0]] not in real:
            real.append([key, value[0]])
        
listseq.append(order2Seq)
loopyt=0
for elem in range(3,75):
    ...
    loopyt=loopyt+len(eval('order'+str(elem)+'Seq'))
...
print(len(real),len(final))
a.close()
filt=open('realdad5.csv','w')
filt.write('Id,Last')
for elem in real:
